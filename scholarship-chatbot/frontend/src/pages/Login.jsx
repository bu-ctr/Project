...
export default function Login() {
  ...
  async function handleGoogleSignIn() {
    ...
    try {
      ...
      console.log("Google Sign-In: ID token obtained");

      // Send token to backend
      console.log("Google Sign-In: Sending token to backend...");
      const res = await api.post("/auth/google-login", { idToken });

      console.log("Google Sign-In: Backend response received");
      localStorage.setItem("token", res.data.token);
      window.dispatchEvent(new Event("auth-change"));

      console.log("Google Sign-In: Success! Redirecting to profile...");
      window.location.href = "/profile";
    } catch (err) {
